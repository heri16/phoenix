...
contract SafeUnwindDepositTests is BaseTest {
    ...
    function test_safeUnwindDeposit_ShouldBurnTokensCorrectly() external __as(BUNDLER3_ADDRESS) {
        ...
        corkAdapter.safeUnwindDeposit(
            ICorkAdapter.SafeUnwindDepositParams({
                poolId: defaultPoolId,
                collateralAssetsOut: unwindAmount,
                owner: address(corkAdapter),
                receiver: alice,
                maxCptAndCstSharesIn: unwindAmount,
    ...
    function test_safeUnwindDeposit_ShouldRevert_WhenSlippageExeceed()
        external
        ...
        __as(BUNDLER3_ADDRESS)
    {
        ...
        corkAdapter.safeUnwindDeposit(
            ICorkAdapter.SafeUnwindDepositParams({
                ...
                owner: address(corkAdapter),
                receiver: alice,
                maxCptAndCstSharesIn: unwindAmount - 1,
                deadline: block.timestamp
            })
        );
    }
