...
contract SafeDepositTests is BaseTest {
    ...
    function test_safeDeposit_ShouldMintTokensCorrectly()
        external
        ...
        __as(BUNDLER3_ADDRESS)
    {
        ...
        corkAdapter.safeDeposit(
            ICorkAdapter.SafeDepositParams({
                poolId: defaultPoolId,
                collateralAssetsIn: depositAmount,
                receiver: alice,
        ...
        assertEq(
            stateAfter.userPrincipalToken - stateBefore.userPrincipalToken,
            depositAmount,
            "User should get correct amount of principal tokens"
        );
        assertEq(
            stateAfter.userSwapToken - stateBefore.userSwapToken,
            depositAmount,
            "User should get correct amount of swap tokens"
        );

        // Verify contract state changes
        assertEq(
            stateAfter.contractCollateral - stateBefore.contractCollateral,
            depositAmount,
            "Contract should get correct amount of collateral assets"
        );
        assertEq(
            stateAfter.principalTokenTotalSupply - stateBefore.principalTokenTotalSupply,
            depositAmount,
            "Correct amount of cPT should get minted"
        );
    }
    ...
    function test_safeDeposit_ShouldRevert_WhenSlippageExeceed()
        external
        __giveAssets(address(corkAdapter))
        __as(BUNDLER3_ADDRESS)
    {
        ...
        corkAdapter.safeDeposit(
            ICorkAdapter.SafeDepositParams({
                ...
                collateralAssetsIn: depositAmount,
                receiver: alice,
                minCptAndCstSharesOut: depositAmount + 100,
                deadline: block.timestamp
            })
        );
    }
