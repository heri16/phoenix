...
contract PreviewAdjustedRateTests is BaseTest {
    ...
    function test_previewAdjustedRate_shouldReturnRateMin_whenOracleReturnsZeroAnswer()
        external
        __as(address(corkPoolManager))
    {
        testOracle.setRate(defaultPoolId, 0);

        uint256 rateMin = constraintRateAdapter.previewAdjustedRate(defaultPoolId);
