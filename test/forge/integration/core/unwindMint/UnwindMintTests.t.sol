...
contract UnwindMintTests is BaseTest {
    ...
    function test_maxUnwindMint_ShouldReturnSameAmount_WhenBalancesAreEqual()
        external
        __as(alice)
    ...
    function test_unwindMint_ShouldNotUseExtra()
        external
        ...
        __deposit(amount, alice)
    {
        ...
        assertGe(
            principalBalanceBefore - principalBalanceAfter,
            1e12,
            "Should not use extra principal shares beyond required amount"
        );
    ...
    function test_unwindMint_ShouldRevert_WhenInsufficientPrincipalTokenBalance()
        external
        __as(alice)
        __deposit(amount, alice)
    {
    ...
    function test_unwindMint_ShouldRevert_WhenInsufficientSwapTokenBalance()
        external
        __as(alice)
        __deposit(amount, alice)
    {
    ...
    function test_unwindMint_ShouldRevert_WhenBelowMinimumShares()
        external
        __createPool(1 days, 6, 18)
        __giveAssets(alice)
        __approveAllTokens(alice, address(corkPoolManager))
    ...
    function test_unwindMint_ShouldWorkWithDifferentDecimalCombinations() external {
        ...
        assertEq(
            collateralReceived,
            TransferHelper.normalizeDecimals(unwindAmount, TARGET_DECIMALS, 8),
            "Should work with 8-18 decimal combination"
        );
    }

    function test_unwindMint_ShouldRevert_WhenRoundingToZero()
        external
        __createPool(1 days, 6, 18)
        __giveAssets(alice)
        __approveAllTokens(alice, address(corkPoolManager))
    ...
    function testFuzz_unwindMint_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals) external {
        ...
        uint256 normalizedDepositAmount =
            TransferHelper.fixedToTokenNativeDecimals(500 ether, collateralAsset.decimals());
        uint256 normalizedUnwindAmount =
            TransferHelper.fixedToTokenNativeDecimals(100 ether, collateralAsset.decimals());
        ...
        assertEq(
            afterSnapshot.userCollateral,
            beforeSnapshot.userCollateral + normalizedUnwindAmount,
            "User collateral balance should increase by unwindAmount"
        );
        assertEq(
            afterSnapshot.userPrincipalToken,
            beforeSnapshot.userPrincipalToken - 100 ether,
            "User principal token balance should decrease by unwindAmount"
        );
        assertEq(
            afterSnapshot.userSwapToken,
            beforeSnapshot.userSwapToken - 100 ether,
            "User swap token balance should decrease by unwindAmount"
        );
        assertEq(afterSnapshot.userRef, beforeSnapshot.userRef, "User reference asset balance should remain unchanged");
        ...
        assertEq(
            afterSnapshot.contractCollateral,
            beforeSnapshot.contractCollateral - normalizedUnwindAmount,
            "Contract collateral balance should decrease by unwindAmount"
        );
        assertEq(
            afterSnapshot.contractRef,
        ...
        assertEq(
            afterSnapshot.principalTokenTotalSupply,
            beforeSnapshot.principalTokenTotalSupply - 100 ether,
            "Principal token total supply should decrease by unwindAmount"
        );
        assertEq(
            afterSnapshot.swapTokenTotalSupply,
            beforeSnapshot.swapTokenTotalSupply - 100 ether,
            "Swap token total supply should decrease by unwindAmount"
        );
        ...
        assertEq(
            afterSnapshot.poolCollateral,
            beforeSnapshot.poolCollateral - normalizedUnwindAmount,
            "Pool locked collateral should decrease by unwindAmount"
        );
        assertEq(afterSnapshot.poolRef, beforeSnapshot.poolRef, "Pool reference asset should remain unchanged");
        ...
        assertEq(
            afterSnapshot.internalState.pool.balances.collateralAsset.locked,
            beforeSnapshot.internalState.pool.balances.collateralAsset.locked - normalizedUnwindAmount,
            "Internal state locked collateral should decrease by unwindAmount"
        );
        assertEq(
            afterSnapshot.internalState.pool.balances.referenceAssetBalance,
    ...
    function testFuzz_previewUnwindMint_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals)
        external
    {
        // Bound decimals to reasonable ranges
        collateralDecimals = uint8(bound(collateralDecimals, 6, 18));
