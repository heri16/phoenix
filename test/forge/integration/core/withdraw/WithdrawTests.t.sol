...
contract WithdrawTests is BaseTest {
    ...
    function test_withdraw_ShouldWork() external __as(alice) __deposit(1000 ether, alice) {
        ...
        (uint256 expectedSharesIn, uint256 expectedCollateralOut, uint256 expectedRefOut) =
            corkPoolManager.previewWithdraw(defaultPoolId, collateralOut);
        ...
        (uint256 sharesIn, uint256 actualCollateralOut, uint256 actualRefOut) =
            corkPoolManager.withdraw(defaultPoolId, collateralOut, alice, alice);
        ...
        assertEq(
            ...
            stateBefore.userCollateral + expectedCollateralOut,
            "User collateral balance should increase"
        );
        ...
        assertEq(
            ...
            stateBefore.userPrincipalToken - expectedSharesIn,
            "User principal token balance should decrease"
        );

        assertEq(
            stateAfter.contractCollateral,
            stateBefore.contractCollateral - expectedCollateralOut,
            "Contract collateral balance should decrease"
        );
        assertEq(
            stateAfter.contractRef,
            stateBefore.contractRef - expectedRefOut,
            "Contract reference balance should decrease"
        );
        assertEq(
            stateAfter.principalTokenTotalSupply,
            stateBefore.principalTokenTotalSupply - expectedSharesIn,
            "Principal token total supply should decrease"
        );

        assertEq(
            stateAfter.poolCollateral,
            stateBefore.poolCollateral - expectedCollateralOut,
            "Pool collateral balance should decrease"
        );
        assertEq(stateAfter.poolRef, stateBefore.poolRef - expectedRefOut, "Pool reference balance should decrease");

        assertEq(
            stateAfter.internalState.shares.withdrawn,
            stateBefore.internalState.shares.withdrawn + expectedSharesIn,
            "Internal state shares withdrawn should increase"
        );
    }
    ...
    function test_withdrawOther_ShouldWork() external {
        ...
        (uint256 expectedSharesIn, uint256 previewActualCollateralOut, uint256 expectedRefOut) =
            corkPoolManager.previewWithdrawOther(defaultPoolId, referenceOut);
        ...
        (uint256 sharesIn, uint256 actualCollateralOut, uint256 actualRefOut) =
            corkPoolManager.withdrawOther(defaultPoolId, referenceOut, alice, alice);
    ...
    function test_PauseWithdrawStatus_blocksWithdrawal() external __as(pauser) {
        ...
        vm.expectRevert(PausableUpgradeable.EnforcedPause.selector);
        corkPoolManager.redeem(defaultPoolId, 0, bravo, bravo);
    }
    ...
    function test_PauseWithdrawStatus_blocksWithdraw() external __as(pauser) {
        ...
        vm.expectRevert(PausableUpgradeable.EnforcedPause.selector);
        corkPoolManager.withdraw(defaultPoolId, 0.01 ether, bravo, bravo);
    }
    ...
    function test_PauseWithdrawStatus_blocksWithdrawOther() external __as(pauser) {
        ...
        vm.expectRevert(PausableUpgradeable.EnforcedPause.selector);
        corkPoolManager.withdrawOther(defaultPoolId, 0.01 ether, bravo, bravo);
    }

    // ================================ Emit Tests ================================ //

    function test_EmitWithdrawShouldRevertWhenCalledByNonOwner() external __as(bravo) {
        IPoolShare.ConstructorParams memory constructorParams = IPoolShare.ConstructorParams({
            poolId: MarketId.wrap(bytes32("")),
            pairName: "Swap Token",
            symbol: "SWT",
        ...
        share.emitWithdraw(bravo, alice, bravo, 1 ether, 0.5 ether);
    }
    ...
    function test_maxWithdraw_consistencyWithActualWithdraw() external __as(alice) __deposit(1 ether, alice) {
        ...
        (uint256 sharesIn, uint256 actualCollateralAssetOut, uint256 actualReferenceAssetOut) =
            corkPoolManager.withdraw(defaultPoolId, maxWithdrawAmount, alice, bob);
    ...
    function test_maxWithdraw_ShouldReturnSameValueAsPoolManager() external __as(bravo) __deposit(1000 ether, bravo) {
    ...
    function test_maxWithdrawOther_ShouldReturnSameValueAsPoolManager()
        external
        __as(alice)
        __deposit(1000 ether, alice)
    {
        uint256 poolManagerResult = corkPoolManager.maxWithdrawOther(defaultPoolId, alice);
        uint256 poolShareResult = swapToken.maxWithdrawOther(alice);

        assertEq(
            poolShareResult, poolManagerResult, "PoolShare maxWithdrawOther should match PoolManager maxWithdrawOther"
        );
    }

    function test_previewWithdraw_ShouldReturnSameValueAsPoolManager()
        external
        __as(bravo)
        __deposit(1000 ether, bravo)
    {
        vm.warp(block.timestamp + 2 days);
    ...
    function test_previewWithdrawOther_ShouldReturnSameValueAsPoolManager()
        external
        __as(alice)
        __deposit(1000 ether, alice)
    {
        ...
        uint256 referenceAssetsOut = 0.01 ether;
        (
            uint256 poolManagerSharesIn,
            uint256 poolManagerActualCollateralAssetOut,
            uint256 poolManagerActualRefAssetOut
        ) = corkPoolManager.previewWithdrawOther(defaultPoolId, referenceAssetsOut);
        (uint256 poolShareSharesIn, uint256 poolShareActualCollateralAssetOut, uint256 poolShareActualRefAssetOut) =
            swapToken.previewWithdrawOther(referenceAssetsOut);

        assertEq(
            poolShareSharesIn,
            poolManagerSharesIn,
            "PoolShare previewWithdrawOther sharesIn should match PoolManager previewWithdrawOther"
        );
        assertEq(
            poolShareActualRefAssetOut,
            poolManagerActualRefAssetOut,
            "PoolShare previewWithdrawOther actualReferenceAssetOut should match PoolManager previewWithdrawOther"
        );
        assertEq(
            poolShareActualCollateralAssetOut,
            poolManagerActualCollateralAssetOut,
            "PoolShare previewWithdrawOther actualCollateralAssetOut should match PoolManager previewWithdrawOther"
        );
    }

    // ================================ Preview Function Tests ================================ //

    function test_previewWithdraw_ShouldReturnCorrectAmounts_WhenExpired()
        external
        __as(alice)
        __deposit(1000 ether, bravo)
    {
    ...
    function test_previewWithdraw_ShouldBeConsistent_BeforeAndAfterLiquiditySeparation()
        external
        __as(alice)
        __deposit(1000 ether, alice)
    {
        // Setup: Create scenario with both collateral and reference assets
        corkPoolManager.swap(defaultPoolId, 200 ether, alice);

        vm.warp(block.timestamp + 2 days); // Expire the pool

        // Test preview BEFORE liquidity separation
        uint256 collateralOut = 100 ether;
        (uint256 sharesBeforeSep, uint256 collateralBeforeSep, uint256 refBeforeSep) =
            corkPoolManager.previewWithdraw(defaultPoolId, collateralOut);

        // Verify liquidity is not separated yet
        StateSnapshot memory stateBefore = _getStateSnapshot(alice, defaultPoolId);
        assertFalse(
            stateBefore.internalState.pool.liquiditySeparated, "Liquidity should not be separated before redeem"
        );

        // Perform a redeem to trigger liquidity separation
        corkPoolManager.redeem(defaultPoolId, 50 ether, alice, alice);

        // Verify liquidity is now separated
        StateSnapshot memory stateAfter = _getStateSnapshot(alice, defaultPoolId);
        assertTrue(stateAfter.internalState.pool.liquiditySeparated, "Liquidity should be separated after redeem");

        // Test preview AFTER liquidity separation
        (uint256 sharesAfterSep, uint256 collateralAfterSep, uint256 refAfterSep) =
            corkPoolManager.previewWithdraw(defaultPoolId, collateralOut);

        // Results should be consistent
        assertEq(sharesBeforeSep, sharesAfterSep, "Shares calculation should be consistent before and after separation");
        assertEq(
            collateralBeforeSep,
            collateralAfterSep,
            "Collateral output should be consistent before and after separation"
        );
        assertEq(refBeforeSep, refAfterSep, "Reference output should be consistent before and after separation");
    }
    ...
    function test_maxWithdrawOther_ShouldReturnCorrectAmount_WhenExpired()
        external
        __as(alice)
        __deposit(500 ether, alice)
    {
        overridePrank(bravo);
    ...
    function test_maxWithdraw_ShouldReturnPoolBalance_WhenOwnerSharesGreaterThanPool()
        external
        __as(alice)
        __deposit(1000 ether, alice)
    {
    ...
    function test_withdraw_ShouldRevert_WhenInsufficientCollateralLiquidity()
        external
        __as(alice)
        __deposit(500 ether, alice)
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IErrors.InsufficientLiquidity.selector, 500 ether, excessiveCollateralOut)
        );
        corkPoolManager.withdraw(defaultPoolId, excessiveCollateralOut, alice, alice);
    }

    function test_withdrawOther_ShouldRevert_WhenInsufficientReferenceLiquidity()
        external
        __as(alice)
        __deposit(500 ether, alice)
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IErrors.InsufficientLiquidity.selector, 250 ether, excessiveReferenceOut)
        );
    ...
    function test_withdrawOther_ShouldRevert_WhenInsufficientAllowance()
        external
        __as(alice)
        __deposit(1000 ether, alice)
    {
    ...
    function test_withdraw_ShouldRevert_WhenExceedsAvailableCollateral()
        external
        __as(alice)
        __deposit(100 ether, alice)
    {
        vm.warp(block.timestamp + 2 days);
    ...
    function test_withdrawOther_ShouldRevert_WhenExceedsAvailableReference()
        external
        __as(alice)
        __deposit(1000 ether, alice)
    {
    ...
    function testFuzz_withdraw_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals) external {
        ...
        uint256 normalizedDepositAmount =
            TransferHelper.fixedToTokenNativeDecimals(1000 ether, collateralAsset.decimals());
        ...
        assertEq(
            ...
            beforeSnapshot.userCollateral + actualCollateralOut,
            "User collateral balance should increase"
        );
        ...
        assertEq(
            ...
            beforeSnapshot.userPrincipalToken - sharesIn,
            "User principal token balance should decrease"
        );
        ...
        assertEq(
            ...
            beforeSnapshot.contractCollateral - actualCollateralOut,
            "Contract collateral balance should decrease"
        );
        assertEq(
            afterSnapshot.contractRef,
            beforeSnapshot.contractRef - actualRefOut,
            "Contract reference balance should decrease"
        );
        ...
        assertEq(
            ...
            beforeSnapshot.principalTokenTotalSupply - sharesIn,
            "Principal token total supply should decrease"
        );
        ...
        assertEq(
            ...
            beforeSnapshot.poolCollateral - actualCollateralOut,
            "Pool collateral balance should decrease"
        );
        ...
        assertEq(
            afterSnapshot.internalState.shares.withdrawn,
            beforeSnapshot.internalState.shares.withdrawn + sharesIn,
            "Internal state shares withdrawn should increase"
        );
    }
    ...
    function testFuzz_withdrawOther_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals) external {
        ...
        uint256 normalizedDepositAmount =
            TransferHelper.fixedToTokenNativeDecimals(1000 ether, collateralAsset.decimals());
        uint256 normalizedExerciseAmount =
            TransferHelper.fixedToTokenNativeDecimals(100 ether, referenceAsset.decimals());
    ...
    function testFuzz_previewWithdraw_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals)
        external
    {
        // Bound decimals to reasonable ranges
        collateralDecimals = uint8(bound(collateralDecimals, 6, 18));
        ...
        // Preview withdraw
        (uint256 previewSharesIn, uint256 previewCollateralOut, uint256 previewRefOut) =
            corkPoolManager.previewWithdraw(defaultPoolId, collateralOut);

        // Execute actual withdraw
        (uint256 actualSharesIn, uint256 actualCollateralOut, uint256 actualRefOut) =
            corkPoolManager.withdraw(defaultPoolId, collateralOut, alice, alice);
    ...
    function testFuzz_previewWithdrawOther_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals)
        external
    {
        // Bound decimals to reasonable ranges
        collateralDecimals = uint8(bound(collateralDecimals, 6, 18));
    ...
    // ================================ Preview Zero Amount Validation Tests ================================ //

    function test_previewWithdraw_ShouldRevert_WhenZeroCollateralOut()
        external
        __as(alice)
        __deposit(1000 ether, alice)
    {
        ...
        corkPoolManager.previewWithdraw(defaultPoolId, 0);
    }

    function test_previewWithdrawOther_ShouldRevert_WhenZeroReferenceOut()
        external
        __as(alice)
        __deposit(1000 ether, alice)
    {
