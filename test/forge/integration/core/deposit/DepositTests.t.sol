...
contract DepositTests is BaseTest {
    ...
    function test_deposit() public __as(alice) {
        ...
        assertEq(
            finalSnapshot.userCollateral,
            initialSnapshot.userCollateral - depositAmount,
            "Depositor collateral balance should decrease"
        );
        assertEq(
            finalSnapshot.contractCollateral,
            initialSnapshot.contractCollateral + depositAmount,
            "CorkPoolManager collateral balance should increase"
        );

        // Verify token minting
        assertEq(
            finalSnapshot.userPrincipalToken,
            initialSnapshot.userPrincipalToken + depositAmount,
            "Principal token balance should increase"
        );
        assertEq(
            finalSnapshot.userSwapToken,
            initialSnapshot.userSwapToken + depositAmount,
            "Swap token balance should increase"
        );
        assertEq(
            finalSnapshot.principalTokenTotalSupply,
            initialSnapshot.principalTokenTotalSupply + depositAmount,
            "Principal token supply should increase"
        );
        assertEq(
            finalSnapshot.swapTokenTotalSupply,
            initialSnapshot.swapTokenTotalSupply + depositAmount,
            "Swap token supply should increase"
        );

        // Verify allowance was consumed
        uint256 initialAllowance = collateralAsset.allowance(alice, address(corkPoolManager));
        assertEq(initialAllowance, type(uint248).max - depositAmount, "Allowance should be consumed");

        // Verify pool internal balances
        assertEq(
            finalSnapshot.poolCollateral,
            initialSnapshot.poolCollateral + depositAmount,
            "Pool collateral locked should increase"
        );
        assertEq(finalSnapshot.poolRef, initialSnapshot.poolRef, "Pool reference asset balance should remain unchanged");
    }
    ...
    function test_previewDeposit() public __as(bravo) {
    ...
    function testFuzz_deposit(uint8 _collateralDecimal, uint8 _referenceDecimal)
        external
        ...
        __approveAllTokens(bravo, address(corkPoolManager))
    {
        ...
        (uint256 initialCollateralLocked, uint256 initialSwapTokenBalance, uint256 initialReferenceAssetBalance) =
            corkPoolManager.getPoolBalances(defaultPoolId);

        {
            ...
            // Verify collateral asset transfers
            assertEq(
                collateralAsset.balanceOf(currentCaller()),
                initialCollateralBalance - depositAmountNormalized,
                "Depositor collateral balance should decrease"
            );
            assertEq(
                collateralAsset.balanceOf(address(corkPoolManager)),
                initialCorkPoolBalance + depositAmountNormalized,
                "CorkPoolManager collateral balance should increase"
            );

            // Verify token minting
            assertEq(
                principalToken.balanceOf(currentCaller()),
                initialPrincipalBalance + 1 ether,
                "Principal token balance should increase"
            );
            assertEq(
                swapToken.balanceOf(currentCaller()), initialSwapBalance + 1 ether, "Swap token balance should increase"
            );
            assertEq(
                principalToken.totalSupply(), initialPrincipalSupply + 1 ether, "Principal token supply should increase"
            );
            assertEq(swapToken.totalSupply(), initialSwapSupply + 1 ether, "Swap token supply should increase");

            // Verify allowance was consumed
            assertEq(
                collateralAsset.allowance(currentCaller(), address(corkPoolManager)),
                initialAllowance - depositAmountNormalized,
                "Allowance should be consumed"
            );
        }

        // Verify pool internal balances
        (uint256 finalCollateralLocked, uint256 finalSwapTokenBalance, uint256 finalReferenceAssetBalance) =
            corkPoolManager.getPoolBalances(defaultPoolId);

        assertEq(
            finalCollateralLocked,
            initialCollateralLocked + depositAmountNormalized,
            "Pool collateral locked should increase"
        );
        assertEq(finalSwapTokenBalance, initialSwapTokenBalance, "Pool swap token balance should remain unchanged");
        assertEq(
            finalReferenceAssetBalance,
            initialReferenceAssetBalance,
            "Pool reference asset balance should remain unchanged"
        );
    }

    function testFuzz_previewDeposit(uint8 _collateralDecimal, uint8 _referenceDecimal)
        external
        __createPoolBounded(1 days, _collateralDecimal, _referenceDecimal)
        __giveAssets(bravo)
        __approveAllTokens(bravo, address(corkPoolManager))
    ...
    function test_deposit_WithDifferentDecimals()
        external
    ...
    function test_isInitialized_ShouldReturnTrue_WhenBothAssetsSet() external {
        uint256 depositAmount = 100 ether;
        uint256 received = corkPoolManager.deposit(defaultPoolId, depositAmount, alice);
        assertEq(received, depositAmount, "Deposit should work when pool is initialized");
    }

    function test_isInitialized_ShouldReturnFalse_WhenPoolNotInitialized() external __as(alice) {
        // Create an uninitialized pool by using a non-existent pool ID
        MarketId uninitializedPoolId = MarketId.wrap(bytes32("0x123"));

        // Trying to deposit to uninitialized pool should revert due to isInitialized check
        vm.expectRevert(IErrors.NotInitialized.selector);
        corkPoolManager.deposit(uninitializedPoolId, 100 ether, alice);
    }
}
