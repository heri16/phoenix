...
contract RedeemTests is BaseTest {
    ...
    function test_redeem() external __as(alice) __deposit(depositAmount, alice) {
        ...
        // Execute redeem
        (uint256 accruedRef, uint256 accruedCollateral) =
        ...
        assertEq(
            _after.userCollateral - before.userCollateral, accruedCollateral, "User should receive collateral assets"
        );

        // Verify principal token state changes
        assertEq(
            before.userPrincipalToken - _after.userPrincipalToken,
            redeemAmount,
            "User principal tokens should be burned"
        );
        assertEq(
            before.principalTokenTotalSupply - _after.principalTokenTotalSupply,
            redeemAmount,
        ...
        assertEq(
            before.contractRef - _after.contractRef, accruedRef, "Contract reference asset balance should decrease"
        );
        assertEq(
            before.contractCollateral - _after.contractCollateral,
            accruedCollateral,
            "Contract collateral asset balance should decrease"
        );

        // Verify pool state changes (balances should be the same regardless of  liquidity separation)
        assertEq(
            _after.poolCollateral, 900 ether, "Pool collateral balance be the same regardless of liquidity separation"
        );
        ...
        assertEq(
            ...
            0,
            "Internal collateral balance should be 0 after liquidity separation"
        );
        assertEq(
            _after.internalState.pool.balances.referenceAssetBalance,
            0,
            "Internal reference balance should be 0 after liquidity separation"
        );
        assertEq(
            _after.internalState.pool.poolArchive.collateralAssetAccrued,
            900 ether,
            "Pool archive collateral should decrease by redeemed amount"
        );
        assertEq(
            _after.internalState.pool.poolArchive.referenceAssetAccrued,
            0,
            "Pool archive reference should decrease by redeemed amount"
        );
    }
    ...
    function test_previewRedeem_ShouldReturnCorrectAmounts_WhenExpired()
        external
        __as(alice)
    ...
    function test_previewRedeem_ShouldReturnZeroZero_WhenZeroAmount() external __as(alice) __deposit(1 ether, alice) {
        vm.warp(block.timestamp + 2 days);

        (uint256 accruedRef, uint256 accruedCollateral) = corkPoolManager.previewRedeem(defaultPoolId, 0);

        assertEq(accruedRef, 0, "Should return 0 reference assets for zero amount");
        assertEq(accruedCollateral, 0, "Should return 0 collateral assets for zero amount");
    }

    function test_previewRedeem_ShouldReturnZeroZero_WhenNotExpired() external __as(alice) __deposit(1 ether, alice) {
        (uint256 accruedRef, uint256 accruedCollateral) = corkPoolManager.previewRedeem(defaultPoolId, 100 ether);

        assertEq(accruedRef, 0, "Should return 0 reference assets when not expired");
        assertEq(accruedCollateral, 0, "Should return 0 collateral assets when not expired");
    }

    function test_previewRedeem_ShouldReturnZeroZero_WhenWithdrawalPaused()
        external
        __as(alice)
        __deposit(1 ether, alice)
    {
        ...
        assertEq(accruedRef, 0, "Should return 0 reference assets when withdrawal paused");
        assertEq(accruedCollateral, 0, "Should return 0 collateral assets when withdrawal paused");
    }

    function test_previewRedeem_ShouldReturnZeroZero_WhenBelowMinimumShares()
        external
        ...
        __as(alice)
    {
        ...
        (uint256 accruedRef, uint256 accruedCollateral) = corkPoolManager.previewRedeem(defaultPoolId, belowMinimum);

        assertEq(accruedRef, 0, "Should return 0 reference assets when below minimum shares");
        assertEq(accruedCollateral, 0, "Should return 0 collateral assets when below minimum shares");
    }

    function test_previewRedeem_ShouldCalculateMinimumSharesWithCollateralDecimals()
        external
        ...
        __as(alice)
    {
        ...
        uint256 minimumShares = 10 ** (18 - 6); // 10^12 when reference asset has lower decimals
        uint256 belowMinimum = minimumShares - 1;

        (uint256 accruedRef, uint256 accruedCollateral) = corkPoolManager.previewRedeem(defaultPoolId, belowMinimum);

        assertEq(accruedRef, 0, "Should return 0 reference assets when below minimum shares (reference lower decimals)");
        assertEq(
            accruedCollateral,
            0,
            "Should return 0 collateral assets when below minimum shares (reference lower decimals)"
        );
    }
    ...
    function test_redeem_ShouldRevert_WhenInsufficientShares()
        external
        __as(alice)
        __deposit(depositAmount, alice)
        __deposit(depositAmount, bob)
    {
        vm.warp(block.timestamp + 2 days);
    ...
    function test_redeem_ShouldRevert_WhenInsufficientSharesAmount()
        external
        __createPool(1 days, 6, 18)
        __giveAssets(alice)
        __approveAllTokens(alice, address(corkPoolManager))
    ...
    function test_redeemWithSameOwnerSender() external __as(alice) __deposit(1 ether, alice) {
        ...
        (uint256 actualReference, uint256 actualCollateral) =
            corkPoolManager.redeem(defaultPoolId, redeemAmount, alice, bob);
        ...
        assertEq(
            receiverCollateralBalanceAfter - receiverCollateralBalanceBefore,
            actualCollateral,
            "Receiver should receive collateral assets"
        );
        assertEq(
            receiverReferenceBalanceAfter - receiverReferenceBalanceBefore,
            actualReference,
            "Receiver should receive reference assets"
        );
        ...
        assertEq(
            ownerCollateralBalanceAfter, ownerCollateralBalanceBefore, "Owner should not receive collateral assets"
        );
    ...
    function test_redeemMultipleReceivers() external __as(alice) __deposit(1 ether, alice) {
        ...
        (uint256 actualReference1, uint256 actualCollateral1) =
            corkPoolManager.redeem(defaultPoolId, redeemAmount / 2, alice, bob);
        (uint256 actualReference2, uint256 actualCollateral2) =
            corkPoolManager.redeem(defaultPoolId, redeemAmount / 2, alice, receiver2);
        ...
        assertEq(
            receiver1CollateralBalanceAfter - receiver1CollateralBalanceBefore,
            actualCollateral1,
            "Receiver1 should receive collateral assets"
        );
        assertEq(
            receiver2CollateralBalanceAfter - receiver2CollateralBalanceBefore,
            actualCollateral2,
            "Receiver2 should receive collateral assets"
        );
    }
    ...
    function test_redeemPreviewConsistency() external __as(alice) __deposit(1 ether, alice) {
        ...
        (uint256 previewReference, uint256 previewCollateral) =
            corkPoolManager.previewRedeem(defaultPoolId, redeemAmount);
    ...
    function testFuzz_redeem(uint8 _collateralDecimal, uint8 _referenceDecimal)
        external
        ...
        __as(alice)
    {
        uint256 depositAmountNormalized =
            TransferHelper.normalizeDecimals(depositAmount, TARGET_DECIMALS, collateralDecimal);
        ...
        // Execute redeem
        (uint256 accruedRef, uint256 accruedCollateral) =
            corkPoolManager.redeem(defaultPoolId, redeemAmountNormalized, alice, alice);
        ...
        assertEq(
            _after.poolCollateral,
            expectedPoolCollateral,
            "Pool collateral balance be the same regardless of liquidity separation"
        );
        assertEq(_after.poolRef, 0, "Pool reference balance be the same regardless of liquidity separation");
    ...
    function testFuzz_previewRedeem(uint8 _collateralDecimal, uint8 _referenceDecimal)
        external
        __createPoolBounded(EXPIRY, _collateralDecimal, _referenceDecimal)
        __giveAssets(alice)
        __approveAllTokens(alice, address(corkPoolManager))
        __as(alice)
    {
        ...
        // Execute actual redeem
        (uint256 actualRef, uint256 actualCollateral) =
        ...
        uint256 expectedCollateral =
            TransferHelper.normalizeDecimals(withdrawAmount, TARGET_DECIMALS, collateralDecimal);
    ...
    function test_fullLifecycle_DepositExerciseRedeem() external __as(alice) {
        ...
        (uint256 accruedRef, uint256 accruedCollateral) =
            corkPoolManager.redeem(defaultPoolId, remainingBalance, alice, alice);
