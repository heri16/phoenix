...
import {
    CompositeRateOracleHelper as Helper,
    MinimalAggregatorV3Interface
} from "test/forge/helpers/CompositeRateOracleHelper.sol";
import {
    btcEthFeed,
    btcUsdFeed,
    daiEthFeed,
    ...
    vaultZero,
    wBtcBtcFeed
} from "test/forge/helpers/Constants.sol";
import {ChainlinkAggregatorMock} from "test/forge/mocks/ChainlinkAggregatorMock.sol";

contract CompositeRateOracleTest is Test {
    using Math for uint256;
    address ensManager = makeAddr("ENS Manager");
    ...
    function testOracleWbtcUsdc() public {
        ...
        CompositeRateOracle oracle = new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, wBtcBtcFeed, btcUsdFeed, 8, vaultZero, 1, usdcUsdFeed, feedZero, 6)
        );
        ...
        uint256 expectedPrice = (uint256(firstBaseAnswer) * uint256(secondBaseAnswer) * 10 ** (36 + 8 + 6 - 8 - 8 - 8))
            / uint256(quoteAnswer);
        assertEq(oracle.price(), expectedPrice);
        assertEq(oracle.rate(), expectedPrice / 1 ether);

        (, int256 latestRoundData,,,) = oracle.latestRoundData();
        assertEq(uint256(latestRoundData), expectedPrice);
    }
    ...
    function testOracleUsdcWbtc() public {
        ...
        CompositeRateOracle oracle = new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, usdcUsdFeed, feedZero, 6, vaultZero, 1, wBtcBtcFeed, btcUsdFeed, 8)
        );
        ...
        uint256 expectedPrice = (uint256(baseAnswer) * 10 ** (36 + 8 + 8 + 8 - 6 - 8))
    ...
    function testOracleWbtcEth() public {
        ...
        CompositeRateOracle oracle = new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, wBtcBtcFeed, btcEthFeed, 8, vaultZero, 1, feedZero, feedZero, 18)
        );
        (, int256 firstBaseAnswer,,,) = wBtcBtcFeed.latestRoundData();
        (, int256 secondBaseAnswer,,,) = btcEthFeed.latestRoundData();
        uint256 expectedPrice = (uint256(firstBaseAnswer) * uint256(secondBaseAnswer) * 10 ** (36 + 18 - 8 - 8 - 18));
        assertEq(oracle.price(), expectedPrice);
        assertEq(oracle.rate(), expectedPrice / 1 ether);
    ...
    function testOracleStEthUsdc() public {
        ...
        CompositeRateOracle oracle = new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, stEthEthFeed, feedZero, 18, vaultZero, 1, usdcEthFeed, feedZero, 6)
        );
        (, int256 baseAnswer,,,) = stEthEthFeed.latestRoundData();
        (, int256 quoteAnswer,,,) = usdcEthFeed.latestRoundData();
        uint256 expectedPrice = uint256(baseAnswer) * 10 ** (36 + 18 + 6 - 18 - 18) / uint256(quoteAnswer);
        assertEq(oracle.price(), expectedPrice);
        assertEq(oracle.rate(), expectedPrice / 1 ether);
    ...
    function testOracleEthUsd() public {
        ...
        CompositeRateOracle oracle = new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, ethUsdFeed, feedZero, 18, vaultZero, 1, feedZero, feedZero, 0)
        );
    ...
    function testOracleStEthEth() public {
        ...
        CompositeRateOracle oracle = new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, stEthEthFeed, feedZero, 18, vaultZero, 1, feedZero, feedZero, 18)
        );
    ...
    function testOracleEthStEth() public {
        ...
        CompositeRateOracle oracle = new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, feedZero, feedZero, 18, vaultZero, 1, stEthEthFeed, feedZero, 18)
        );
        (, int256 expectedPrice,,,) = stEthEthFeed.latestRoundData();
    ...
    function testOracleUsdcUsd() public {
        ...
        CompositeRateOracle oracle = new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, usdcUsdFeed, feedZero, 6, vaultZero, 1, feedZero, feedZero, 0)
        );
    ...
    function testSDaiEthOracle() public {
        ...
        CompositeRateOracle oracle = new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(sDaiVault, 1e18, daiEthFeed, feedZero, 18, vaultZero, 1, feedZero, feedZero, 18)
        );
    ...
    function testEthSDaiOracle() public {
        ...
        CompositeRateOracle oracle = new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, feedZero, feedZero, 18, sDaiVault, 1e18, daiEthFeed, feedZero, 18)
        );
    ...
    function testConstructorZeroVaultConversionSample() public {
        ...
        new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(sDaiVault, 0, daiEthFeed, feedZero, 18, vaultZero, 1, usdcEthFeed, feedZero, 6)
        );
        vm.expectRevert(ICompositeRateOracle.VaultConversionSampleIsZero.selector);
        new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, daiEthFeed, feedZero, 18, sDaiVault, 0, usdcEthFeed, feedZero, 6)
        );
    }
    ...
    function testConstructorVaultZeroNotOneSample(uint256 vaultConversionSample) public {
        ...
        new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 0, daiEthFeed, feedZero, 18, vaultZero, 1, usdcEthFeed, feedZero, 6)
        );
        vm.expectRevert(ICompositeRateOracle.VaultConversionSampleIsNotOne.selector);
        new CompositeRateOracle(
            ensManager,
            Helper.makeSourceParamsArr(vaultZero, 1, daiEthFeed, feedZero, 18, vaultZero, 0, usdcEthFeed, feedZero, 6)
        );
    }

    function testConstructor_ShouldRevertWhenEnsManagerIsZeroAddress() public {
        require(block.chainid == 1, "test only runs on Ethereum mainnet");

        vm.expectRevert(abi.encodeWithSelector(Ownable.OwnableInvalidOwner.selector, address(0)));
        new CompositeRateOracle(
            address(0),
            Helper.makeSourceParamsArr(vaultZero, 1, daiEthFeed, feedZero, 18, vaultZero, 1, usdcEthFeed, feedZero, 6)
        );
    }
}
