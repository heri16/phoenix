...
contract SigUtils is Test {
    ...
    function getCutomStructHash(CustomPermit memory _permit) internal pure returns (bytes32) {
        return keccak256(abi.encode(CUSTOM_PERMIT_TYPEHASH, _permit.owner, _permit.spender, _permit.value, _permit.nonce, _permit.deadline, _permit.functionHash));
    }
    ...
    function getCustomTypedDataHash(CustomPermit memory _permit, bytes32 DOMAIN_SEPARATOR) internal pure returns (bytes32) {
    ...
    function getCustomPermit(address owner, address spender, uint256 value, uint256 nonce, uint256 deadline, uint256 pk, bytes32 DOMAIN_SEPARATOR, string memory functionName) internal pure returns (bytes memory) {
        bytes32 functionHash = keccak256(bytes(functionName));
        CustomPermit memory permit = CustomPermit({owner: owner, spender: spender, value: value, nonce: nonce, deadline: deadline, functionHash: functionHash});
    ...
    function getPermit(address owner, address spender, uint256 value, uint256 nonce, uint256 deadline, uint256 pk, bytes32 DOMAIN_SEPARATOR) internal pure returns (bytes memory) {
