...
contract ReentrancyAttacker {
    CorkPoolManager private target;
    ...
    function attack(uint256 _attackType) external {
        ...
        if (attackType == 1) {
            ...
            // Attack exercise
            target.exercise(marketId, 1 ether, address(this));
            ...
            // Attack withdraw
            target.withdraw(marketId, 1 ether, address(this), address(this));
            ...
            target.unwindExercise(marketId, 1 ether, address(this));
        }
    }
    ...
    function onTransfer() external {
        if (attacking) {
            ...
            else if (attackType == 3) target.exercise(marketId, 1 ether, address(this));
            ...
            else if (attackType == 7) target.withdraw(marketId, 1 ether, address(this), address(this));
            ...
            else if (attackType == 10) target.unwindExercise(marketId, 1 ether, address(this));
        }
    }
...
contract CorkPoolReentrancyTest is Helper {
    ...
    function setUp() public {
        ...
        attacker = new ReentrancyAttacker(address(corkPoolManager));
        ...
        maliciousMarketId = corkPoolManager.getId(market);
    ...
    function test_exercise_ShouldPreventReentrancy() external {
        ...
        vm.startPrank(address(attacker));
        maliciousCollateral.approve(address(corkPoolManager), type(uint256).max);
        maliciousReference.approve(address(corkPoolManager), type(uint256).max);
    ...
    function test_unwindExercise_ShouldPreventReentrancy() external {
        ...
        maliciousReference.approve(address(corkPoolManager), type(uint256).max);
        corkPoolManager.exercise(maliciousMarketId, 100 ether, address(attacker));
