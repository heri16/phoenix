...
contract TreasuryFeesTest is Helper {
    ...
    function setUp() public {
        ...
        defaultCorkController.setTreasury(treasury);

        // Set fees for testing
        defaultCorkController.updateUnwindSwapFeeRate(marketId, FEE_PERCENTAGE);
        defaultCorkController.updateSwapFeePercentage(marketId, FEE_PERCENTAGE);
    ...
    function test_unwindSwap_ShouldSendFeesToTreasury() external {
        ...
        uint256 unwindAmount = 10.526_315_79 ether;
        ...
        assertEq(treasuryFeesReceived, unwindFee, "Treasury should receive the exact fee amount");
        assertApproxEqAbs(treasuryFeesReceived, 0.526_315_79 ether, 0.0001 ether, "Treasury should receive the exact fee amount");
    }
    ...
    function test_unwindExercise_ShouldSendFeesToTreasury() external {
        ...
        assertApproxEqAbs(treasuryFeesReceived, 2.631_578_95 ether, 0.001 ether, "this tests depends on https://github.com/Cork-Technology/Depeg-Swap-Private/pull/51 to succeed");
    }
    ...
    function test_exercise_ShouldSendFeesToTreasury() external {
        ...
        vm.startPrank(user);
        (,, uint256 fee) = corkPoolManager.exercise(marketId, exerciseShares, user);
        vm.stopPrank();
    ...
    function test_swap_ShouldSendFeesToTreasury() external {
        ...
        (uint256 shares, uint256 compensation, uint256 fee) = corkPoolManager.swap(marketId, assets, address(44));
        vm.stopPrank();

        assertApproxEqAbs(shares, 105.263_157_9 ether, 0.001 ether);
        assertApproxEqAbs(compensation, 105.263_157_9 ether, 0.001 ether);

        uint256 treasuryBalanceAfter = collateralAsset.balanceOf(treasury);
        uint256 treasuryFeesReceived = treasuryBalanceAfter - treasuryBalanceBefore;
        assertApproxEqAbs(treasuryFeesReceived, 5.263_157_9 ether, 0.0001 ether);
    }
    ...
    function testFuzz_unwindSwap_ShouldSendFeesToTreasury(uint8 raDecimals, uint8 paDecimals) external {
        ...
        corkPoolManager.swap(marketId, swapAmount, user); // Create pool liquidity

        uint256 treasuryBalanceBefore = collateralAsset.balanceOf(treasury);

        // Use a larger amount for testing
        uint256 unwindAmount = TransferHelper.normalizeDecimals(10.526_315_79 ether, TARGET_DECIMALS, raDecimals);
        ...
        assertEq(treasuryFeesReceived, unwindFee, "Treasury should receive the exact fee amount");
        uint256 expectedAmount = TransferHelper.normalizeDecimals(0.526_315_79 ether, TARGET_DECIMALS, raDecimals);
    ...
    function testFuzz_unwindExercise_ShouldSendFeesToTreasury(uint8 raDecimals, uint8 paDecimals) external {
        ...
        uint256 swapAmount = TransferHelper.normalizeDecimals(200 ether, TARGET_DECIMALS, raDecimals);
        corkPoolManager.deposit(marketId, depositAmount, currentCaller());
        corkPoolManager.swap(marketId, swapAmount, user); // Create liquidity

        uint256 treasuryBalanceBefore = collateralAsset.balanceOf(treasury);
        uint256 shares = 50 ether;

        corkPoolManager.unwindExercise(marketId, shares, user);

        uint256 treasuryBalanceAfter = collateralAsset.balanceOf(treasury);
        uint256 treasuryFeesReceived = treasuryBalanceAfter - treasuryBalanceBefore;
        // this tests depends on https://github.com/Cork-Technology/Depeg-Swap-Private/pull/51 to succeed
        uint256 expectedAmount = TransferHelper.normalizeDecimals(2.631_578_95 ether, TARGET_DECIMALS, raDecimals);
    ...
    function testFuzz_swap_ShouldSendFeesToTreasury(uint8 raDecimals, uint8 paDecimals) external {
        ...
        (uint256 shares, uint256 compensation, uint256 fee) = corkPoolManager.swap(marketId, TransferHelper.normalizeDecimals(100 ether, TARGET_DECIMALS, raDecimals), address(44));

        {
            uint256 expectedShares = 105.263_157_9 ether;
            uint256 expectedCompensation = TransferHelper.normalizeDecimals(105.263_157_9 ether, TARGET_DECIMALS, paDecimals);
        ...
        uint256 treasuryBalanceAfter = collateralAsset.balanceOf(treasury);
        uint256 treasuryFeesReceived = treasuryBalanceAfter - treasuryBalanceBefore;
        uint256 expectedFeeAmount = TransferHelper.normalizeDecimals(5.263_157_9 ether, TARGET_DECIMALS, raDecimals);
