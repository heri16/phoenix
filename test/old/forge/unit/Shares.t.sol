...
contract SharesTest is Helper {
    ...
    function test_ConstructorShouldWorkCorrectly() external {
        (, address _swapToken) = corkPoolManager.shares(poolId);
        share = PoolShare(_swapToken);
        ...
        // PoolShare
        assertEq(share.pairName(), "DWETH-DWETH1CST");
        assertEq(address(share.poolManager()), address(corkPoolManager));
        assertEq(share.factory(), address(sharesFactory));
        assertEq(share.owner(), address(corkPoolManager));

        // ERC20
        assertEq(share.totalSupply(), depositAmount);
        assertEq(share.balanceOf(DEFAULT_ADDRESS), depositAmount);
        assertEq(share.allowance(DEFAULT_ADDRESS, address(corkPoolManager)), 0);
        assertEq(share.decimals(), 18);
        assertEq(share.symbol(), "DWETH1CST");
        assertEq(share.name(), "DWETH-DWETH1CST");
    }
    ...
    function test_getReservesShouldReturnCorrectReserves() external {
        ...
        PoolShare(principalToken).approve(address(corkPoolManager), type(uint128).max);
        corkPoolManager.redeem(defaultCurrencyId, depositAmount, DEFAULT_ADDRESS, DEFAULT_ADDRESS);
