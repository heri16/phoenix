...
contract CorkAdapterWithdrawTest is Helper {
    ...
    function setupDifferentDecimals(uint8 raDecimals, uint8 paDecimals) internal returns (uint8, uint8) {
        ...
        defaultCorkController.updateSwapFeePercentage(defaultCurrencyId, 2 ether);
    ...
    function setupWithShares() internal {
        ...
        principalToken.approve(address(corkAdapter), type(uint256).max);
    ...
    function test_withdraw_collateralOnly_success_approval() public {
        ...
        corkAdapter.safeWithdraw(ICorkAdapter.SafeWithdrawParams({poolId: defaultCurrencyId, collateralAssetsOut: collateralOut, owner: DEFAULT_ADDRESS, receiver: RECEIVER, maxCptSharesIn: cptBalance, deadline: block.timestamp}));
    ...
    function test_withdraw_collateralOnly_success_transferred() public {
        ...
        // because cork adapter has the balance, we set it as the owner
        corkAdapter.safeWithdraw(ICorkAdapter.SafeWithdrawParams({poolId: defaultCurrencyId, collateralAssetsOut: collateralOut, owner: address(corkAdapter), receiver: RECEIVER, maxCptSharesIn: cptBalance, deadline: block.timestamp}));
    ...
    function test_withdrawOther_referenceOnly_success() public {
        vm.startPrank(DEFAULT_ADDRESS);
        setupWithShares();

        uint256 collateralOut = 0; // collateral out is 0 because we are using safeWithdrawOther function
        ...
        corkAdapter.safeWithdrawOther(ICorkAdapter.SafeWithdrawOtherParams({poolId: defaultCurrencyId, referenceAssetsOut: referenceOut, owner: DEFAULT_ADDRESS, receiver: RECEIVER, maxCptSharesIn: cptBalance, deadline: block.timestamp}));
    ...
    function test_withdraw_revertsOnCollateralAssetIsZero() public {
        vm.startPrank(DEFAULT_ADDRESS);
        setupWithShares();
        ...
        corkAdapter.safeWithdraw(ICorkAdapter.SafeWithdrawParams({poolId: defaultCurrencyId, collateralAssetsOut: collateralOut, owner: DEFAULT_ADDRESS, receiver: RECEIVER, maxCptSharesIn: 1000e18, deadline: block.timestamp}));
    ...
    function test_withdraw_revertsOnZeroReceiver() public {
        ...
        corkAdapter.safeWithdraw(ICorkAdapter.SafeWithdrawParams({poolId: defaultCurrencyId, collateralAssetsOut: 100e18, owner: DEFAULT_ADDRESS, receiver: address(0), maxCptSharesIn: 1000e18, deadline: block.timestamp}));
    ...
    function test_withdraw_revertsOnZeroOwner() public {
        ...
        corkAdapter.safeWithdraw(ICorkAdapter.SafeWithdrawParams({poolId: defaultCurrencyId, collateralAssetsOut: 100e18, owner: address(0), receiver: RECEIVER, maxCptSharesIn: 1000e18, deadline: block.timestamp}));
    ...
    function test_withdraw_revertsOnExpiredDeadline() public {
        ...
        corkAdapter.safeWithdraw(ICorkAdapter.SafeWithdrawParams({poolId: defaultCurrencyId, collateralAssetsOut: 100e18, owner: DEFAULT_ADDRESS, receiver: RECEIVER, maxCptSharesIn: 1000e18, deadline: block.timestamp - 1}));
    ...
    function test_withdraw_revertsOnSlippageExceeded() public {
        ...
        corkAdapter.safeWithdraw(ICorkAdapter.SafeWithdrawParams({poolId: defaultCurrencyId, collateralAssetsOut: collateralOut, owner: DEFAULT_ADDRESS, receiver: RECEIVER, maxCptSharesIn: maxSharesIn, deadline: block.timestamp}));
    ...
    function test_withdraw_revertsOnUnauthorizedCaller() public {
        ...
        corkAdapter.safeWithdraw(ICorkAdapter.SafeWithdrawParams({poolId: defaultCurrencyId, collateralAssetsOut: 100e18, owner: DEFAULT_ADDRESS, receiver: RECEIVER, maxCptSharesIn: 1000e18, deadline: block.timestamp}));
    }
    ...
    function testFuzz_withdraw_differentDecimals(uint8 raDecimals, uint8 paDecimals, uint256 collateralOut) public {
        ...
        Balances memory before = Balances({
            periphery: principalToken.balanceOf(address(corkAdapter)),
    ...
    function test_balanceConsistency_withdraw() public {
        ...
        uint256 peripheryCollateralBefore = collateralAsset.balanceOf(address(corkAdapter));
        uint256 defaultAddressCollateralBefore = collateralAsset.balanceOf(DEFAULT_ADDRESS);
        uint256 receiverCollateralBefore = collateralAsset.balanceOf(RECEIVER);
        uint256 corkCollateralBefore = collateralAsset.balanceOf(address(corkPoolManager));

        corkAdapter.safeWithdraw(ICorkAdapter.SafeWithdrawParams({poolId: defaultCurrencyId, collateralAssetsOut: collateralOut, owner: DEFAULT_ADDRESS, receiver: RECEIVER, maxCptSharesIn: cptBalance, deadline: block.timestamp}));

        uint256 totalCollateralSupplyAfter = collateralAsset.totalSupply();
        uint256 peripheryCollateralAfter = collateralAsset.balanceOf(address(corkAdapter));
        uint256 defaultAddressCollateralAfter = collateralAsset.balanceOf(DEFAULT_ADDRESS);
        uint256 receiverCollateralAfter = collateralAsset.balanceOf(RECEIVER);
        uint256 corkCollateralAfter = collateralAsset.balanceOf(address(corkPoolManager));
