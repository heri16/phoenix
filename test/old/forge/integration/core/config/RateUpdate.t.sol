...
contract RateUpdateTest is Helper {
    function setUp() external {
        ...
        vm.startPrank(address(corkPoolManager));
    }
    ...
    function test_shouldUpdateRateDownCorrectly() external {
        ...
        rate = corkPoolManager.swapRate(defaultCurrencyId);

        vm.assertEq(rate, 0.9 ether);
    }

    function test_constraintRateAdapter_bootstrap() external {
        (uint256 _lastAdjustedRate, uint256 _lastAdjustmentTimestamp, uint256 _remainingCredits) = constraintRateAdapter.constraints(defaultCurrencyId);
    ...
    function test_constraintRateAdapter_adjustedRate_withinCreditLimit() external {
        testOracle.setRate(defaultCurrencyId, 1.0005 ether);

        uint256 adjustedRate = constraintRateAdapter.adjustedRate(defaultCurrencyId);
    ...
    function test_constraintRateAdapter_adjustedRate_exceedsCreditLimit() external {
        ...
        adjustedRate = constraintRateAdapter.adjustedRate(defaultCurrencyId);
        ...
        (_lastAdjustedRate, _lastAdjustmentTimestamp, _remainingCredits) = constraintRateAdapter.constraints(defaultCurrencyId);
        vm.assertEq(_lastAdjustedRate, 1.001 ether);
        vm.assertEq(_remainingCredits, 0);
    }

    function test_constraintRateAdapter_adjustedRate_hitMinBoundary() external {
    ...
    function test_constraintRateAdapter_adjustedRate_hitMaxBoundary() external {
    ...
    function test_constraintRateAdapter_adjustedRate_noChangeWhenSameRate() external {
        (uint256 _lastAdjustedRate, uint256 _lastAdjustmentTimestamp, uint256 _remainingCredits) = constraintRateAdapter.constraints(defaultCurrencyId);
        ...
        (uint256 __lastAdjustedRate, uint256 __lastAdjustmentTimestamp, uint256 after_remainingCredits) = constraintRateAdapter.constraints(defaultCurrencyId);
        vm.assertEq(after_remainingCredits, _remainingCredits);
    }

    function test_constraintRateAdapter_creditRefillOverTime() external {
        testOracle.setRate(defaultCurrencyId, 1.1 ether);
    ...
    function test_constraintRateAdapter_previewAdjustedRate() external {
        testOracle.setRate(defaultCurrencyId, 1.0005 ether);

        uint256 previewRate = constraintRateAdapter.previewAdjustedRate(defaultCurrencyId);
    ...
    function test_constraintRateAdapter_onlyCorkPoolManagerModifier() external {
        vm.stopPrank();
    ...
    function test_constraintRateAdapter_multipleRateChanges() external {
        testOracle.setRate(defaultCurrencyId, 1.03 ether);
        uint256 rate1 = constraintRateAdapter.adjustedRate(defaultCurrencyId);
        vm.assertEq(rate1, 1.03 ether);

        testOracle.setRate(defaultCurrencyId, 1.06 ether);
        uint256 rate2 = constraintRateAdapter.adjustedRate(defaultCurrencyId);
        vm.assertEq(rate2, 1.06 ether);

        testOracle.setRate(defaultCurrencyId, 1.11 ether);
        uint256 rate3 = constraintRateAdapter.adjustedRate(defaultCurrencyId);
    ...
    function test_constraintRateAdapter_downwardRateChange() external {
    ...
    function test_rateBelowMinimumShouldNotConsumeCredits() external {
        testOracle.setRate(defaultCurrencyId, 0.9 ether);
        ...
        testOracle.setRate(defaultCurrencyId, 0.89 ether);

        // trigger update rate
        constraintRateAdapter.adjustedRate(defaultCurrencyId);

        (,, uint256 remainingCreditsBefore) = constraintRateAdapter.constraints(defaultCurrencyId);

        // trigger second update rate
        constraintRateAdapter.adjustedRate(defaultCurrencyId);

        (,, uint256 remainingCreditsAfter) = constraintRateAdapter.constraints(defaultCurrencyId);

        assertEq(remainingCreditsAfter, remainingCreditsBefore);
    }

    function test_rateAboveMaximumShouldNotConsumeCredits() external {
        testOracle.setRate(defaultCurrencyId, 1.1 ether);
