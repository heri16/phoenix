...
contract CreateNewPoolTest is BaseTest {
    ...
    function test_CreateNewPool_ShouldRevert_WhenCalledByNonPoolDeployer() public __as(alice) {
        ...
        vm.expectRevert(abi.encodeWithSignature("AccessControlUnauthorizedAccount(address,bytes32)", alice, defaultCorkController.POOL_CREATOR_ROLE()));
    ...
    function test_CreateNewPool_ShouldRevert_WhenRateMinIsZero() public {
        Market memory market = Market({collateralAsset: address(collateralAsset), referenceAsset: address(referenceAsset), expiryTimestamp: block.timestamp + 1, rateOracle: address(testOracle), rateMin: 0, rateMax: 0, rateChangePerDayMax: 0, rateChangeCapacityMax: 0});
