...
import {InvalidERC20} from "test/new/forge/mocks/InvalidERC20.sol";
...
contract SharesFactoryTest is BaseTest {
    ...
    function test_ConstructorShouldRevertWithZeroAddress() external {
        vm.expectRevert(abi.encodeWithSelector(IErrors.ZeroAddress.selector));
        new SharesFactory(address(0));
    }
    ...
    function test_getDeployedSwapSharesShouldWorkWithDifferentMarketParameters() external {
        ...
        defaultCorkController.createNewPool(IDefaultCorkController.PoolCreationParams(pool2, DEFAULT_REVERSE_SWAP_FEE, DEFAULT_BASE_REDEMPTION_FEE, false));
    ...
    function createPool() internal returns (MarketId _poolId) {
        ...
        defaultCorkController.createNewPool(IDefaultCorkController.PoolCreationParams(poolParams, DEFAULT_REVERSE_SWAP_FEE, DEFAULT_BASE_REDEMPTION_FEE, false));
    }
}
