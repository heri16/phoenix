...
contract SafeSwapTests is BaseTest {
    ...
    function test_safeSwap_ShouldSwapTokensCorrectly() external __as(BUNDLER3_ADDRESS) {
        ...
        corkAdapter.safeSwap(ICorkAdapter.SafeSwapParams({poolId: defaultPoolId, collateralAssetsOut: swapAmount, receiver: alice, maxCstSharesIn: swapAmount, maxReferenceAssetsIn: swapAmount, deadline: block.timestamp}));
    ...
    function test_safeSwap_ShouldRevert_WhenCalledByNonBundler3() external __as(alice) {
        ...
        corkAdapter.safeSwap(ICorkAdapter.SafeSwapParams({poolId: defaultPoolId, collateralAssetsOut: swapAmount, receiver: address(0), maxCstSharesIn: swapAmount, maxReferenceAssetsIn: swapAmount, deadline: block.timestamp}));
    }
    ...
    function test_safeSwap_ShouldRevert_WhenDeadlinePassed() external __as(BUNDLER3_ADDRESS) {
        ...
        corkAdapter.safeSwap(ICorkAdapter.SafeSwapParams({poolId: defaultPoolId, collateralAssetsOut: swapAmount, receiver: alice, maxCstSharesIn: swapAmount, maxReferenceAssetsIn: swapAmount, deadline: block.timestamp - 1}));
    }
    ...
    function test_safeSwap_ShouldRevert_WhenZeroAmount() external __as(BUNDLER3_ADDRESS) {
        ...
        corkAdapter.safeSwap(ICorkAdapter.SafeSwapParams({poolId: defaultPoolId, collateralAssetsOut: 0, receiver: alice, maxCstSharesIn: swapAmount, maxReferenceAssetsIn: swapAmount, deadline: block.timestamp}));
    }

    function test_safeSwap_ShouldRevert_WhenSlippageExeceed() external __giveAssets(address(corkAdapter)) __as(BUNDLER3_ADDRESS) {
        // When maxCstSharesIn is too low
        vm.expectRevert(ErrorsLib.SlippageExceeded.selector);
        corkAdapter.safeSwap(ICorkAdapter.SafeSwapParams({poolId: defaultPoolId, collateralAssetsOut: swapAmount, receiver: alice, maxCstSharesIn: swapAmount - 1, maxReferenceAssetsIn: type(uint256).max, deadline: block.timestamp}));

        // When maxReferenceAssetsIn is too low
        vm.expectRevert(ErrorsLib.SlippageExceeded.selector);
        corkAdapter.safeSwap(ICorkAdapter.SafeSwapParams({poolId: defaultPoolId, collateralAssetsOut: swapAmount, receiver: alice, maxCstSharesIn: type(uint256).max, maxReferenceAssetsIn: swapAmount - 1, deadline: block.timestamp}));
    }
