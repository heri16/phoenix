...
contract SafeExerciseOtherTests is BaseTest {
    ...
    function test_safeExerciseOther_ShouldWorkCorrectly() external __as(BUNDLER3_ADDRESS) {
        ...
        corkAdapter.safeExerciseOther(ICorkAdapter.SafeExerciseOtherParams({poolId: defaultPoolId, referenceAssetsIn: exerciseAmount, receiver: alice, minCollateralAssetsOut: exerciseAmount, maxCstSharesIn: exerciseAmount, deadline: block.timestamp}));
        ...
        assertEq(stateAfter.contractCollateral, stateBefore.contractCollateral - exerciseAmount, "Contract should sent correct amount of collateral assets");
        assertEq(stateAfter.contractRef, stateBefore.contractRef + exerciseAmount, "Contract should take correct amount of reference assets");
    }
    ...
    function test_safeExerciseOther_ShouldRevert_WhenCalledByNonBundler3() external __as(alice) {
        ...
        corkAdapter.safeExerciseOther(ICorkAdapter.SafeExerciseOtherParams({poolId: defaultPoolId, referenceAssetsIn: exerciseAmount, receiver: address(0), minCollateralAssetsOut: exerciseAmount, maxCstSharesIn: exerciseAmount, deadline: block.timestamp}));
    }
    ...
    function test_safeExerciseOther_ShouldRevert_WhenDeadlinePassed() external __as(BUNDLER3_ADDRESS) {
        ...
        corkAdapter.safeExerciseOther(ICorkAdapter.SafeExerciseOtherParams({poolId: defaultPoolId, referenceAssetsIn: exerciseAmount, receiver: alice, minCollateralAssetsOut: exerciseAmount, maxCstSharesIn: exerciseAmount, deadline: block.timestamp - 1}));
    }
    ...
    function test_safeExerciseOther_ShouldRevert_WhenZeroAmount() external __as(BUNDLER3_ADDRESS) {
        ...
        corkAdapter.safeExerciseOther(ICorkAdapter.SafeExerciseOtherParams({poolId: defaultPoolId, referenceAssetsIn: 0, receiver: alice, minCollateralAssetsOut: exerciseAmount, maxCstSharesIn: exerciseAmount, deadline: block.timestamp}));
    }

    function test_safeExerciseOther_ShouldRevert_WhenSlippageExeceed() external __giveAssets(address(corkAdapter)) __as(BUNDLER3_ADDRESS) {
        // When minCollateralAssetsOut is too high
        vm.expectRevert(ErrorsLib.SlippageExceeded.selector);
        corkAdapter.safeExerciseOther(ICorkAdapter.SafeExerciseOtherParams({poolId: defaultPoolId, referenceAssetsIn: exerciseAmount, receiver: alice, minCollateralAssetsOut: exerciseAmount + 1, maxCstSharesIn: type(uint256).max, deadline: block.timestamp}));

        // When maxCstSharesIn is too low
        vm.expectRevert(ErrorsLib.SlippageExceeded.selector);
        corkAdapter.safeExerciseOther(ICorkAdapter.SafeExerciseOtherParams({poolId: defaultPoolId, referenceAssetsIn: exerciseAmount, receiver: alice, minCollateralAssetsOut: 0, maxCstSharesIn: exerciseAmount - 1, deadline: block.timestamp}));
    }
