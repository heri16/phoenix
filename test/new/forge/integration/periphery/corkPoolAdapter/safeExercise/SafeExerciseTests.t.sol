...
contract SafeExerciseTests is BaseTest {
    ...
    function test_safeExercise_ShouldWorkCorrectly() external __as(BUNDLER3_ADDRESS) {
        ...
        corkAdapter.safeExercise(ICorkAdapter.SafeExerciseParams({poolId: defaultPoolId, cstSharesIn: swapAmount, receiver: alice, minCollateralAssetsOut: swapAmount, maxReferenceAssetsIn: swapAmount, deadline: block.timestamp}));
        ...
        assertEq(stateAfter.contractCollateral, stateBefore.contractCollateral - swapAmount, "Contract should sent correct amount of collateral assets");
        assertEq(stateAfter.contractRef, stateBefore.contractRef + swapAmount, "Contract should take correct amount of reference assets");
    }
    ...
    function test_safeExercise_ShouldRevert_WhenCalledByNonBundler3() external __as(alice) {
        ...
        corkAdapter.safeExercise(ICorkAdapter.SafeExerciseParams({poolId: defaultPoolId, cstSharesIn: swapAmount, receiver: address(0), minCollateralAssetsOut: swapAmount, maxReferenceAssetsIn: swapAmount, deadline: block.timestamp}));
    }
    ...
    function test_safeExercise_ShouldRevert_WhenDeadlinePassed() external __as(BUNDLER3_ADDRESS) {
        ...
        corkAdapter.safeExercise(ICorkAdapter.SafeExerciseParams({poolId: defaultPoolId, cstSharesIn: swapAmount, receiver: alice, minCollateralAssetsOut: swapAmount, maxReferenceAssetsIn: swapAmount, deadline: block.timestamp - 1}));
    }
    ...
    function test_safeExercise_ShouldRevert_WhenZeroAmount() external __as(BUNDLER3_ADDRESS) {
        ...
        corkAdapter.safeExercise(ICorkAdapter.SafeExerciseParams({poolId: defaultPoolId, cstSharesIn: 0, receiver: alice, minCollateralAssetsOut: swapAmount, maxReferenceAssetsIn: swapAmount, deadline: block.timestamp}));
    }

    function test_safeExercise_ShouldRevert_WhenSlippageExeceed() external __giveAssets(address(corkAdapter)) __as(BUNDLER3_ADDRESS) {
        // When minCollateralAssetsOut is too high
        vm.expectRevert(ErrorsLib.SlippageExceeded.selector);
        corkAdapter.safeExercise(ICorkAdapter.SafeExerciseParams({poolId: defaultPoolId, cstSharesIn: swapAmount, receiver: alice, minCollateralAssetsOut: swapAmount + 1, maxReferenceAssetsIn: type(uint256).max, deadline: block.timestamp}));

        // When maxReferenceAssetsIn is too low
        vm.expectRevert(ErrorsLib.SlippageExceeded.selector);
        corkAdapter.safeExercise(ICorkAdapter.SafeExerciseParams({poolId: defaultPoolId, cstSharesIn: swapAmount, receiver: alice, minCollateralAssetsOut: 0, maxReferenceAssetsIn: swapAmount - 1, deadline: block.timestamp}));
    }
