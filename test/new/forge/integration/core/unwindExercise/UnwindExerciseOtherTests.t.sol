...
contract UnwindExerciseOtherTests is BaseTest {
    ...
    function test_unwindExerciseOtherBasic_ShouldWorkCorrectly() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
        ...
        (uint256 expectedCollateralAssetsIn, uint256 expectedCstSharesOut, uint256 expectedFee) = corkPoolManager.previewUnwindExerciseOther(defaultPoolId, desiredReferenceAssetsOut);

        vm.expectEmit(true, true, true, true);
        emit IPoolManager.PoolSwap(defaultPoolId, alice, alice, expectedCollateralAssetsIn, expectedCstSharesOut, 0, 0, true);
        ...
        (uint256 collateralAssetsIn, uint256 refAssetsOut, uint256 fee) = corkPoolManager.unwindExerciseOther(defaultPoolId, desiredReferenceAssetsOut, alice);
        ...
        assertEq(stateAfter.userSwapToken, stateBefore.userSwapToken + desiredReferenceAssetsOut, "User should recieve correct amount of cst shares");
        ...
        assertEq(stateAfter.internalState.pool.balances.swapTokenBalance, stateBefore.internalState.pool.balances.swapTokenBalance - desiredReferenceAssetsOut, "Swap token balance should decrease by shares spent");
    ...
    function test_unwindExerciseOther_ShouldWorkWithDifferentOwnerReceiver() external __as(alice) __depositAndSwap(depositAmount, swapAmount, alice) {
        overridePrank(bob);
        ...
        (uint256 collateralAssetsIn, uint256 cstSharesOut, uint256 fee) = corkPoolManager.unwindExerciseOther(defaultPoolId, desiredReferenceAssetsOut, alice);
    ...
    function test_previewUnwindExercise_ShouldReturnCorrectAmounts() external {
        ...
        (uint256 collateralAssetsIn, uint256 cstSharesOut, uint256 fee) = corkPoolManager.previewUnwindExercise(defaultPoolId, referenceAssetsOut);
    ...
    function test_previewUnwindExerciseOther_ShouldReturnSameValueAsPoolManager() external __as(alice) __depositAndSwap(depositAmount, swapAmount, alice) {
        uint256 assets = 1 ether;

        (uint256 poolManagerCaAssetsIn, uint256 poolManagerCstSharesOut, uint256 poolManagerFee) = corkPoolManager.previewUnwindExerciseOther(defaultPoolId, assets);
        (uint256 poolShareCaAssetsIn, uint256 poolShareCstSharesOut, uint256 poolShareFee) = swapToken.previewUnwindExerciseOther(assets);

        assertEq(poolShareCaAssetsIn, poolManagerCaAssetsIn, "PoolShare previewUnwindExerciseOther sharesIn should match PoolManager previewUnwindExerciseOther");
        assertEq(poolShareCstSharesOut, poolManagerCstSharesOut, "PoolShare previewUnwindExerciseOther cstSharesOut should match PoolManager previewUnwindExerciseOther");
        assertEq(poolShareFee, poolManagerFee, "PoolShare previewUnwindExerciseOther fee should match PoolManager previewUnwindExerciseOther");
    }
    ...
    function test_previewUnwindExercise_ShouldReturnZero_AfterExpiry() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
        ...
        (uint256 collateralAssetsIn, uint256 cstSharesOut, uint256 fee) = corkPoolManager.previewUnwindExercise(defaultPoolId, 100 ether);
    ...
    function test_maxUnwindExerciseOther_ShouldReturnSameValueAsPoolManager() external __as(alice) __depositAndSwap(depositAmount, swapAmount, alice) {
    ...
    function test_maxUnwindExerciseOther_ShouldReturnZero_AfterExpiry() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
        uint256 expiry = swapToken.expiry();
        vm.warp(expiry + 1);
    ...
    function test_unwindExerciseOther_ShouldRevert_WhenInsufficientReferenceAssetLiquidity() external __as(alice) __depositAndSwap(400 ether, 200 ether, alice) {
        ...
        overridePrank(alice);
        vm.expectRevert(abi.encodeWithSignature("InsufficientLiquidity(uint256,uint256)", referenceAssets, unwindAmount));
        corkPoolManager.unwindExerciseOther(defaultPoolId, unwindAmount, alice);
    }

    function test_unwindExerciseOther_ShouldRevert_WhenInsufficientSwapTokenLiquidity() external __as(alice) __depositAndSwap(400 ether, 200 ether, alice) {
    ...
    function test_unwindExerciseOther_ShouldRevertAfterExpiry() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
        uint256 expiry = swapToken.expiry();
        vm.warp(expiry + 1);
    ...
    function test_unwindExerciseOther_ShouldRevert_WhenGloballyPaused() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
    ...
    function test_unwindExerciseOther_ShouldRevert_WhenInsufficientCollateralAssetBalance() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
    ...
    function test_unwindExerciseOther_ShouldRevert_WhenReceiverIsZeroAddress() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
    ...
    function testFuzz_unwindExerciseOther_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals) external {
        ...
        uint256 desiredReferenceAssetsOut = TransferHelper.fixedToTokenNativeDecimals(0.5 ether, referenceAsset.decimals());
        ...
        assertEq(afterSnapshot.userRef, beforeSnapshot.userRef + desiredReferenceAssetsOut, "User should receive correct amount of reference asset compensation");
        ...
        assertEq(afterSnapshot.contractRef, beforeSnapshot.contractRef - desiredReferenceAssetsOut, "Contract should send reference asset desiredReferenceAssetsOut");
        ...
        assertEq(afterSnapshot.poolRef, beforeSnapshot.poolRef - desiredReferenceAssetsOut, "Pool reference assets should decrease by desiredReferenceAssetsOut");
        ...
        assertEq(afterSnapshot.internalState.pool.balances.referenceAssetBalance, beforeSnapshot.internalState.pool.balances.referenceAssetBalance - desiredReferenceAssetsOut, "Reference asset balance should decrease by refAssetsOut");
    ...
    function testFuzz_previewUnwindExerciseOther_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals) external {
        // Bound decimals to reasonable ranges
        collateralDecimals = uint8(bound(collateralDecimals, 6, 18));
        referenceDecimals = uint8(bound(referenceDecimals, 6, 18));
        ...
        (uint256 previewCollateralAssetsIn, uint256 previewCstSharesOut, uint256 previewFee) = corkPoolManager.previewUnwindExerciseOther(defaultPoolId, desiredReferenceAssetsOut);

        // Execute actual unwind exercise other
        (uint256 actualCollateralAssetsIn, uint256 actualRefAssetsOut, uint256 actualFee) = corkPoolManager.unwindExerciseOther(defaultPoolId, desiredReferenceAssetsOut, alice);
