...
contract UnwindDepositTests is BaseTest {
    ...
    function test_maxUnwindDeposit_ShouldReturnSameAmount_WhenBalancesAreEqual() external __as(alice) __deposit(1000 ether, alice) {
    ...
    function test_maxUnwindDeposit_ShouldReturnSameValueAsPoolManager() external __as(alice) __deposit(1000 ether, alice) {
    ...
    function testFuzz_unwindDeposit_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals) external {
        ...
        // ================================ User State Changes ================================ //
        assertEq(afterSnapshot.userCollateral, beforeSnapshot.userCollateral + normalizedDepositAmount, "User collateral balance should increase by unwindAmount");
        assertEq(afterSnapshot.userPrincipalToken, beforeSnapshot.userPrincipalToken - 1 ether, "User principal token balance should decrease by unwindAmount");
        assertEq(afterSnapshot.userSwapToken, beforeSnapshot.userSwapToken - 1 ether, "User swap token balance should decrease by unwindAmount");
        assertEq(afterSnapshot.userRef, beforeSnapshot.userRef, "User reference asset balance should remain unchanged");

        // ================================ Contract State Changes ================================ //
        assertEq(afterSnapshot.contractCollateral, beforeSnapshot.contractCollateral - normalizedDepositAmount, "Contract collateral balance should decrease by unwindAmount");
        assertEq(afterSnapshot.contractRef, beforeSnapshot.contractRef, "Contract reference asset balance should remain unchanged");

        // ================================ Token Supply Changes ================================ //
        assertEq(afterSnapshot.principalTokenTotalSupply, beforeSnapshot.principalTokenTotalSupply - 1 ether, "Principal token total supply should decrease by unwindAmount");
        assertEq(afterSnapshot.swapTokenTotalSupply, beforeSnapshot.swapTokenTotalSupply - 1 ether, "Swap token total supply should decrease by unwindAmount");

        // ================================ Pool Internal State Changes ================================ //
        assertEq(afterSnapshot.poolCollateral, beforeSnapshot.poolCollateral - normalizedDepositAmount, "Pool locked collateral should decrease by unwindAmount");
        assertEq(afterSnapshot.poolRef, beforeSnapshot.poolRef, "Pool reference asset should remain unchanged");

        // ================================ Internal State Consistency ================================ //
        assertEq(afterSnapshot.internalState.pool.balances.collateralAsset.locked, beforeSnapshot.internalState.pool.balances.collateralAsset.locked - normalizedDepositAmount, "Internal state locked collateral should decrease by unwindAmount");
        assertEq(afterSnapshot.internalState.pool.balances.referenceAssetBalance, beforeSnapshot.internalState.pool.balances.referenceAssetBalance, "Internal state locked reference asset should remain unchanged");
    }
    ...
    function test_unwindDeposit_ShouldRevert_WhenInsufficientPrincipalTokenBalance() external __as(alice) __deposit(amount, alice) {
    ...
    function test_unwindDeposit_ShouldRevert_WhenInsufficientSwapTokenBalance() external __as(alice) __deposit(amount, alice) {
    ...
    function testFuzz_previewUnwindDeposit_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals) external {
        // Bound decimals to reasonable ranges
        collateralDecimals = uint8(bound(collateralDecimals, 6, 18));
        referenceDecimals = uint8(bound(referenceDecimals, 6, 18));
