...
contract UnwindSwapTests is BaseTest {
    ...
    function test_unwindSwapBasic_ShouldWorkCorrectly() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
        ...
        (uint256 expectedCstSharesOut, uint256 expectedRefAssetsOut, uint256 expectedFee) = corkPoolManager.previewUnwindSwap(defaultPoolId, desiredAssetsIn);

        (uint256 cstSharesOut, uint256 refAssetsOut, uint256 fee) = corkPoolManager.unwindSwap(defaultPoolId, desiredAssetsIn, alice);
        ...
        assertEq(stateAfter.userCollateral, stateBefore.userCollateral - desiredAssetsIn, "User should spend exact collateral amount");
        ...
        assertEq(stateAfter.userSwapToken, stateBefore.userSwapToken + cstSharesOut, "User should recieve correct amount of cst shares");
        ...
        assertEq(stateAfter.contractCollateral, stateBefore.contractCollateral + desiredAssetsIn - fee, "Contract collateral balance should increase by assets out excluding fee");
        ...
        assertEq(stateAfter.poolCollateral, stateBefore.poolCollateral + (desiredAssetsIn - fee), "Pool collateral should increase by assets out minus fee");
        ...
        assertEq(stateAfter.internalState.pool.balances.collateralAsset.locked, stateBefore.internalState.pool.balances.collateralAsset.locked + (desiredAssetsIn - fee), "Locked collateral should increase by assets out minus fee");
        ...
        assertEq(stateAfter.internalState.pool.balances.swapTokenBalance, stateBefore.internalState.pool.balances.swapTokenBalance - cstSharesOut, "Swap token balance should decrease by shares spent");
    ...
    function test_unwindSwap_ShouldEmitcorrectEvents() external __as(alice) __depositAndSwap(depositAmount, swapAmount, alice) {
        ...
        (uint256 shares, uint256 compensation, uint256 fee) = corkPoolManager.unwindSwap(defaultPoolId, assetAmount, alice);
    ...
    function test_unwindSwap_ShouldWorkWithDifferentOwnerReceiver() external __as(alice) __depositAndSwap(depositAmount, swapAmount, alice) {
        overridePrank(bob);
        ...
        (uint256 shares, uint256 compensation, uint256 fee) = corkPoolManager.unwindSwap(defaultPoolId, desiredAssetsIn, alice);
    ...
    function test_previewUnwindSwap_ShouldReturnSameValueAsPoolManager() external __as(alice) __depositAndSwap(depositAmount, swapAmount, alice) {
        uint256 assets = 1 ether;

        (uint256 poolManagerSharesIn, uint256 poolManagerCompensation, uint256 poolManagerFee) = corkPoolManager.previewUnwindSwap(defaultPoolId, assets);
        (uint256 poolShareSharesIn, uint256 poolShareCompensation, uint256 poolShareFee) = swapToken.previewUnwindSwap(assets);

        assertEq(poolShareSharesIn, poolManagerSharesIn, "PoolShare previewUnwindSwap sharesIn should match PoolManager previewUnwindSwap");
        assertEq(poolShareCompensation, poolManagerCompensation, "PoolShare previewUnwindSwap compensation should match PoolManager previewUnwindSwap");
        assertEq(poolShareFee, poolManagerFee, "PoolShare previewUnwindSwap fee should match PoolManager previewUnwindSwap");
    }

    function test_previewUnwindSwap_ShouldReturnZero_AfterExpiry() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
        uint256 expiry = swapToken.expiry();
        vm.warp(expiry + 1);

        (uint256 shares, uint256 compensation, uint256 fee) = corkPoolManager.previewUnwindSwap(defaultPoolId, 0.5 ether);
    ...
    function test_previewUnwindSwap_ShouldReturnZero_WhenPaused() external __as(address(defaultCorkController)) {
        ...
        (uint256 shares, uint256 compensation, uint256 fee) = corkPoolManager.previewUnwindSwap(defaultPoolId, 100 ether);
        assertEq(shares, 0, "Should return zero shares");
        assertEq(compensation, 0, "Should return zero compensation");
        assertEq(fee, 0, "Should return zero fee");
    }
    ...
    function test_maxUnwindSwap_ShouldReturnSameValueAsPoolManager() external __as(alice) __depositAndSwap(depositAmount, swapAmount, alice) {
    ...
    function test_maxUnwindSwap_ShouldReturnZero_AfterExpiry() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
        uint256 expiry = swapToken.expiry();
        vm.warp(expiry + 1);
    ...
    function test_unwindSwap_ShouldRevert_WhenInsufficientReferenceAssetLiquidity() external __as(alice) __depositAndSwap(400 ether, 200 ether, alice) {
        ...
        (uint256 previewSwapAsset, uint256 previewReferenceAsset,) = corkPoolManager.previewUnwindSwap(defaultPoolId, unwindAmount);
    ...
    function test_unwindSwap_ShouldRevert_WhenInsufficientSwapTokenLiquidity() external __as(alice) __depositAndSwap(400 ether, 200 ether, alice) {
    ...
    function test_unwindSwap_ShouldRevert_WhenGloballyPaused() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
    ...
    function test_unwindSwap_ShouldRevert_WhenInsufficientCollateralAssetBalance() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
        ...
        vm.expectRevert(abi.encodeWithSignature("ERC20InsufficientBalance(address,uint256,uint256)", alice, 10_101_010_101_010_102, 0.5 ether));
        corkPoolManager.unwindSwap(defaultPoolId, 0.5 ether, alice);
    }

    function test_unwindSwap_ShouldRevert_WhenReceiverIsZeroAddress() external __as(alice) __depositAndSwap(2 ether, 1 ether, alice) {
    ...
    function testFuzz_unwindSwap_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals) external {
        ...
        // ================================ User State Changes ================================ //
        assertEq(afterSnapshot.userCollateral, beforeSnapshot.userCollateral - desiredAssetsIn, "User should spend exact collateral amount");
        ...
        assertEq(afterSnapshot.userSwapToken, beforeSnapshot.userSwapToken + cstSharesOut, "User should receive correct amount of cst shares");
        ...
        assertEq(afterSnapshot.contractCollateral, beforeSnapshot.contractCollateral + desiredAssetsIn - fee, "Contract collateral balance should increase by assets in excluding fee");
        ...
        assertEq(afterSnapshot.poolCollateral, beforeSnapshot.poolCollateral + (desiredAssetsIn - fee), "Pool collateral should increase by assets in minus fee");
        ...
        assertEq(afterSnapshot.internalState.pool.balances.collateralAsset.locked, beforeSnapshot.internalState.pool.balances.collateralAsset.locked + (desiredAssetsIn - fee), "Locked collateral should increase by assets in minus fee");
        ...
        assertEq(afterSnapshot.internalState.pool.balances.swapTokenBalance, beforeSnapshot.internalState.pool.balances.swapTokenBalance - cstSharesOut, "Swap token balance should decrease by shares spent");
    ...
    function testFuzz_previewUnwindSwap_WithDifferentDecimals(uint8 collateralDecimals, uint8 referenceDecimals) external {
        // Bound decimals to reasonable ranges
        collateralDecimals = uint8(bound(collateralDecimals, 6, 18));
        referenceDecimals = uint8(bound(referenceDecimals, 6, 18));
        ...
        // Preview unwind swap
        (uint256 previewCstSharesOut, uint256 previewRefAssetsOut, uint256 previewFee) = corkPoolManager.previewUnwindSwap(defaultPoolId, desiredAssetsIn);

        // Execute actual unwind swap
        (uint256 actualCstSharesOut, uint256 actualRefAssetsOut, uint256 actualFee) = corkPoolManager.unwindSwap(defaultPoolId, desiredAssetsIn, alice);
