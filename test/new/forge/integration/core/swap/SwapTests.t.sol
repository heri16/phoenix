...
contract SwapTests is BaseTest {
    ...
    function test_swapBasic() external __as(alice) __deposit(1 ether, alice) {
        ...
        (uint256 expectedShares, uint256 expectedCompensation, uint256 expectedFee) = corkPoolManager.previewSwap(defaultPoolId, desiredAssets);
        ...
        assertEq(stateAfter.userCollateral, stateBefore.userCollateral + desiredAssets, "User should receive exact collateral amount");
        assertEq(stateAfter.userRef, stateBefore.userRef - compensation, "User should spend reference asset compensation");
        ...
        assertEq(stateAfter.contractCollateral, stateBefore.contractCollateral - (desiredAssets + fee), "Contract collateral balance should decrease by assets out plus fee");
        assertEq(stateAfter.contractRef, stateBefore.contractRef + compensation, "Contract should receive reference asset compensation");

        // Assert pool asset tracking
        assertEq(stateAfter.poolCollateral, stateBefore.poolCollateral - (desiredAssets + fee), "Pool collateral should decrease by assets out plus fee");
        assertEq(stateAfter.poolRef, stateBefore.poolRef + compensation, "Pool reference assets should increase by compensation");

        // Assert internal state changes
        assertEq(stateAfter.internalState.pool.balances.collateralAsset.locked, stateBefore.internalState.pool.balances.collateralAsset.locked - (desiredAssets + fee), "Locked collateral should decrease by assets out plus fee");
        assertEq(stateAfter.internalState.pool.balances.referenceAssetBalance, stateBefore.internalState.pool.balances.referenceAssetBalance + compensation, "Reference asset balance should increase by compensation");
        assertEq(stateAfter.internalState.pool.balances.swapTokenBalance, stateBefore.internalState.pool.balances.swapTokenBalance + shares, "Swap token balance should increase by shares received");
    }

    function test_swap_shouldNotReleaseFreeCollateral() external __createPool(1 days, 6, 18) __giveAssets(DEFAULT_ADDRESS) __approveAllTokens(DEFAULT_ADDRESS, address(corkPoolManager)) __as(DEFAULT_ADDRESS) {
        defaultCorkController.updateSwapFeePercentage(defaultPoolId, 99_999_900_001_000);

        corkPoolManager.mint(defaultPoolId, 100 ether, DEFAULT_ADDRESS);

        uint256 collateralOut = 1e6;

        (uint256 shares, uint256 compensation, uint256 fee) = corkPoolManager.swap(defaultPoolId, collateralOut, DEFAULT_ADDRESS);
    ...
    function test_previewSwapWithSwapToken() external __as(alice) __deposit(1 ether, alice) {
        ...
        BalanceSnapshot memory beforeBalances = BalanceSnapshot({collateralAsset: collateralAsset.balanceOf(alice), referenceAsset: referenceAsset.balanceOf(alice), swapToken: swapToken.balanceOf(alice), principalToken: principalToken.balanceOf(alice)});

        PreviewswapVars memory preview;
        (preview.collateralAsset, preview.swapToken, preview.fee) = corkPoolManager.previewExercise(defaultPoolId, swapAmount);
        preview.rate = corkPoolManager.swapRate(defaultPoolId);

        PreviewswapVars memory actual;
        (actual.collateralAsset, actual.swapToken, actual.fee) = corkPoolManager.exerciseOther(defaultPoolId, swapAmount, alice);
        actual.rate = corkPoolManager.swapRate(defaultPoolId);

        BalanceSnapshot memory afterBalances = BalanceSnapshot({collateralAsset: collateralAsset.balanceOf(alice), referenceAsset: referenceAsset.balanceOf(alice), swapToken: swapToken.balanceOf(alice), principalToken: principalToken.balanceOf(alice)});
    ...
    function testFuzz_swap(uint8 _collateralDecimal, uint8 _referenceDecimal) external __createPoolBounded(EXPIRY, _collateralDecimal, _referenceDecimal) __giveAssets(DEFAULT_ADDRESS) __approveAllTokens(DEFAULT_ADDRESS, address(corkPoolManager)) {
        ...
        (uint256 shares, uint256 compensation, uint256 fee) = corkPoolManager.swap(defaultPoolId, desiredAssets, currentCaller());
    ...
    function testFuzz_swapPrincipalToken(uint8 _collateralDecimal, uint8 _referenceDecimal, uint256 rate) external __createPoolBounded(EXPIRY, _collateralDecimal, _referenceDecimal) __giveAssets(DEFAULT_ADDRESS) __approveAllTokens(DEFAULT_ADDRESS, address(corkPoolManager)) __as(DEFAULT_ADDRESS) {
        rate = bound(rate, 0.9 ether, 1 ether);

        defaultCorkController.updateSwapFeePercentage(defaultPoolId, 0);
        testOracle.setRate(defaultPoolId, rate);

        depositAmount = TransferHelper.normalizeDecimals(depositAmount, TARGET_DECIMALS, collateralDecimal);

        uint256 received = _deposit(defaultPoolId, depositAmount, currentCaller());

        uint256 swapAmount = 0.5 ether * 1e18 / rate;

        swapAmount = TransferHelper.normalizeDecimals(swapAmount, TARGET_DECIMALS, referenceDecimal);

        (received,,) = corkPoolManager.exerciseOther(defaultPoolId, swapAmount, DEFAULT_ADDRESS);
        uint256 expiry = swapToken.expiry();
        vm.warp(expiry + 1);

        (uint256 accruedReferenceAsset, uint256 accruedCollateralAsset) = corkPoolManager.redeem(defaultPoolId, 1 ether, DEFAULT_ADDRESS, DEFAULT_ADDRESS);

        uint256 expectedAmount = TransferHelper.normalizeDecimals(0.5 ether, TARGET_DECIMALS, collateralDecimal);
        uint256 acceptableDelta = TransferHelper.normalizeDecimals(1, TARGET_DECIMALS, collateralDecimal);

        vm.assertApproxEqAbs(received, expectedAmount, acceptableDelta);
    }

    function testFuzz_previewSwap(uint8 _collateralDecimal, uint8 _referenceDecimal) external __createPoolBounded(EXPIRY, _collateralDecimal, _referenceDecimal) __giveAssets(DEFAULT_ADDRESS) __approveAllTokens(DEFAULT_ADDRESS, address(corkPoolManager)) {
        uint256 depositAmountNormalized = TransferHelper.normalizeDecimals(1000 ether, TARGET_DECIMALS, collateralDecimal);
        uint256 desiredAssets = TransferHelper.normalizeDecimals(0.5 ether, TARGET_DECIMALS, collateralDecimal);
        ...
        // Preview swap
        (uint256 previewShares, uint256 previewCompensation, uint256 previewFee) = corkPoolManager.previewSwap(defaultPoolId, desiredAssets);

        // Execute actual swap
        (uint256 actualShares, uint256 actualCompensation, uint256 actualFee) = corkPoolManager.swap(defaultPoolId, desiredAssets, currentCaller());
    ...
    function test_previewSwap_ShouldReturnSameValueAsPoolManager() external __as(alice) __deposit(depositAmount, alice) {
        uint256 assets = 1 ether;

        (uint256 poolManagerSharesOut, uint256 poolManagerCompensation, uint256 poolManagerFee) = corkPoolManager.previewSwap(defaultPoolId, assets);
    ...
    function test_swap_ShouldRevert_WhenInsufficientReferenceAssetBalance() external __as(alice) __deposit(1 ether, alice) {
    ...
    function test_swap_ShouldRevert_WhenRateOracleReturnsInvalidData() external __as(alice) __deposit(10 ether, alice) {
        ...
        overridePrank(alice);
        (uint256 cstSharesIn, uint256 referenceAssetsIn, uint256 fee) = corkPoolManager.swap(defaultPoolId, 1 ether, alice);
    ...
    function test_swap_EdgeCase_VerySmallAmount() external __as(alice) __deposit(1 ether, alice) {
        ...
        overridePrank(alice);
        (uint256 shares, uint256 compensation, uint256 fee) = corkPoolManager.swap(defaultPoolId, verySmallAmount, alice);
